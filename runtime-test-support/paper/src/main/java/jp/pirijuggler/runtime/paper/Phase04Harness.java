...
public final class Phase04Harness {
    ...
    private void tick(){
        ...
        try {if(!Files.exists(file))return;var p=Bukkit.getPlayerExact("PiriRuntimeTest");if(p==null||!production.canUseSlot(p.getUniqueId()))return;
            ...
            var state=session.publicState();state.addProperty("gameState",mode.equals("BONUS_ENTRY")?"BONUS_ENTRY_SPINNING":"NORMAL_SPINNING");state.addProperty("lampOn",mode.equals("BONUS_ENTRY"));state.addProperty("expectedNextClientSequence",lastSequence+1);state.addProperty("stoppedMask",0);state.add("displayStops",stops(display));send(p,Envelope.current(PacketType.PUBLIC_STATE,state));
