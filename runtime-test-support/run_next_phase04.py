...
try:
 ...
 public=cli().get('publicState') or {}; check('GOD presentation did not choose result',session().get('bonus_type')=='BIG' and public.get('gameState')=='BIG_READY',{'serverBonus':session().get('bonus_type'),'clientGameState':public.get('gameState'),'serverGameState':session().get('game_state')}); manifest['passed']=True
