...
public final class StopCatalogue {
    public record Evaluation(StopTriplet stops,int winningGrapeLines,int winningBellLines,int winningPieroLines,int winningReplayLines,
        ...
        public int lineMask(DisplayRole role){return switch(role){case GRAPE->winningGrapeLines;case BELL->winningBellLines;case PIERO->winningPieroLines;case REPLAY->winningReplayLines;case BIG_ENTRY->winningBigLines;case REG_ENTRY->winningRegLines;default->0;};}
        public int totalLines(){return Integer.bitCount(winningGrapeLines)+Integer.bitCount(winningBellLines)+Integer.bitCount(winningPieroLines)+Integer.bitCount(winningReplayLines)+Integer.bitCount(winningBigLines)+Integer.bitCount(winningRegLines);}
        ...
        public int targetRank(DisplayRole role){if(!valid(role))throw new IllegalArgumentException("Not a strict candidate");return switch(role){case MISS->5;case CHERRY->leftTopCherry?1:2;case PREMIUM_B->0;default->Integer.numberOfTrailingZeros(lineMask(role));};}
    }
    ...
    public static Evaluation evaluate(StopTriplet stops){
        return new Evaluation(stops,winning(stops,GRAPE,GRAPE,GRAPE),winning(stops,BELL,BELL,BELL),winning(stops,PIERO,PIERO,PIERO),winning(stops,REPLAY,REPLAY,REPLAY),
            FixedReels.row(Reel.LEFT,stops.left(),-1)==CHERRY,FixedReels.row(Reel.LEFT,stops.left(),0)==CHERRY,FixedReels.row(Reel.LEFT,stops.left(),1)==CHERRY,winning(stops,SEVEN,SEVEN,SEVEN),winning(stops,SEVEN,SEVEN,BAR));
    }
