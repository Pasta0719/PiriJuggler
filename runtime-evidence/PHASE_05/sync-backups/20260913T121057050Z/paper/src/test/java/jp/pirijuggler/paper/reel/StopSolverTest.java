        ...
        for(var role:DisplayRole.values()){
            ...
            for(var e:CATALOGUE.candidates(role))for(int mask=0;mask<7;mask++)if(seen.add(e.stops().fixedKey(mask))){
                ...
                for(var reel:Reel.values())if((mask&reel.bit())==0)for(int p=0;p<21;p++){
                    final int press=p;var best=candidates.stream().min(Comparator.comparingInt((StopCatalogue.Evaluation c)->Math.floorMod(c.stops().stop(reel)-press,21)).thenComparingInt(c->c.targetRank(role)).thenComparingInt(c->c.stops().left()).thenComparingInt(c->c.stops().center()).thenComparingInt(c->c.stops().right())).orElseThrow();
