...
EVIDENCE = ROOT / "runtime-evidence/PHASE_04"
...
def start_client(name):
    ...
    cmd=["cmd.exe","/d","/c",str(ROOT/"gradlew.bat"),"-PruntimeAcceptance=true",f"-PruntimeScenario={name}",f"-PruntimeRun={RUN}", "-PruntimeEvidencePhase=PHASE_04",":runtime-test-client:runClient","--console=plain"]
...
try:
    ...
    check("startup verifies 9261 candidates and exhaustive solver",verification["allSequences"]==500094 and verification["premiumFSequences"]==166698 and verification["premiumFSecondChecks"]==7938,verification)
    roles=["GRAPE","BELL","PIERO","REPLAY","CHERRY","MISS","BIG_ENTRY","REG_ENTRY","PREMIUM_B"]
    ...
    cases=[(role,False,order) for role in roles for order in orders]+[(role,True,order) for role in ["MISS","CHERRY","PIERO"] for order in orders]
    observed=[]
    for index,(role,premium,order) in enumerate(cases,1):
        ...
        correct=all(p["reel"]==r and p["spinId"]==row["spinId"] and p["stopIndex"]==row["finalStops"][r.lower()] and 0<=p["pressedIndex"]<21 and p["slip"]==(p["stopIndex"]-p["pressedIndex"])%21 and p["durationMs"]==min(1200,80+50*p["slip"]) for p,r in zip(stops,order))
    ...
    check("72 runtime rounds cover every role and stop order",len(rounds())==72 and len(observed)==216)
