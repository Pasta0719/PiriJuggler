...
public final class MachineService implements Listener, CommandExecutor {
    ...
    public MachineService(PiriJugglerPlugin plugin, Map<String, Object> config) {
        ...
        plugin.executors().database(() -> {
            ...
            return database.open(jvm, now, config, new SplittableRandom(new SecureRandom().nextLong()), plugin.getLogger()::warning);
        }, (loaded, error) -> {
    ...
    public void receive(Player player, Envelope envelope) {
        ...
        if (envelope.packetType() != PacketType.CLOSE_REQUEST) { error(player, "INVALID_STATE"); return; }
        try {
            JsonObject body = envelope.payload();
            if (body.size() != 3) throw new IllegalArgumentException();
            ...
            closeRequest(player, id, machine, sequence);
    ...
    private void closeRequest(Player player, UUID id, int machine, long sequence) {
        ...
        submit(player, owner, machine, () -> database.closeSession(owner, id, machine, sequence, System.currentTimeMillis(), graceMs), reason -> {
    ...
    public void disconnect(UUID player) {
        ...
        submit(null, player, session.machine(), () -> { database.disconnect(player, System.currentTimeMillis(), graceMs); return null; }, unused -> {});
    }
