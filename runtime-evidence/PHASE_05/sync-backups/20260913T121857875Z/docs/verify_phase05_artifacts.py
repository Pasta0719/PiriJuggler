...
results={phase:read(root/f'runtime-evidence/{phase}/result.json') for phase in ['PHASE_05','PHASE_05_PHASE01_REGRESSION','PHASE_05_REEL_REGRESSION']}
...
summary={'passed':True,'tests':counts,'totalTests':sum(counts.values()),'imageCount':len(images),'soundEventCount':len(registry),'suppliedAudio':supplied,'runtimeAssertions':len(results['PHASE_05']['assertions']),'phase01Regression':True,'reelRegression':True,'buildHashes':results['PHASE_05']['buildHashes']}
