...
public final class SlotInput {
    ...
    private final Predicate<PacketType> allowed;
    private final Set<Integer> held=new HashSet<>();private final Map<String,Long> clicked=new HashMap<>();
    private Long closeAt;
    public SlotInput(ClientSession session,Consumer<Envelope> sender,LongSupplier nanos){this(session,sender,nanos,ignored->true);}
    public SlotInput(ClientSession session,Consumer<Envelope> sender,LongSupplier nanos,Predicate<PacketType> allowed){this.session=session;this.sender=sender;time=nanos;this.allowed=allowed;}
