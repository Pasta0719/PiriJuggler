...
| CHERRY | 1,510 |
| MISS | 5,242 |
| BONUS（MISS形＋厳密BAR確定形） | 5,250 |
| BIG_ENTRY | 10 |
| REG_ENTRY | 10 |
| PREMIUM_B | 774 |

BAR-BAR-BARは5ラインで別途評価します。厳密な0枚BAR確定形は8件で、単独BIG/REGは同じBONUS候補集合を使います。MISSと小役・入賞候補にはBAR揃いを含めません。REG入賞形はSEVEN-SEVEN-BARのままです。
...
`ReelEngine` は起動時に候補最低数、通常555,660通り、F 166,698通りの全停止順・全押下indexを検証します。Fの第2停止7,938条件を含みます。失敗時は診断をSEVEREへ記録し、台利用を拒否します。
...
Phase04は停止制御の部品までです。抽選、BET・払出し、ゲーム状態の永続化、プレミアム演出の制御と音event送信は後続Phaseのゲームコントローラーが接続します。Phase05が通常ゲームへ接続済みです。
...
実機ランナーはPhase01でSHAを確認済みのPaper 1.21-130を使用し、localhost:25588の専用サーバーと実Fabricクライアントを起動します。10表示役×6停止順とFの3表示役×6停止順、計78回転・234 STOPを検証します。テスト専用controllerが役を選び、実際のキー入力とパケットを本番の `ReelRound` に渡します。helperは配布JARへ含まれません。テストでの役指定や表示は資産移動を行いません。
