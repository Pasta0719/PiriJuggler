...
- Rear-heaven policy accepted: manage rear mode independently from the six front modes. While in rear-heaven, apply the published enhanced GG behavior for lower-yellow triggers and preserve the published multi-stock-oriented reward character; GG obtained through this rear-mode route does not receive an additional loop stock where source-backed. Unpublished rear-heaven entry, fall, and stock-count distribution cells are PIRI_SPECIFIC Phase-04 tuning values and must not be flattened away merely to hit payout.
