...
results={phase:read(root/f'runtime-evidence/{phase}/result.json') for phase in ['PHASE_05','PHASE_05_PHASE01_REGRESSION','PHASE_05_REEL_REGRESSION','PHASE_05_BAR_BIG','PHASE_05_BAR_REG']}
...
reels=results['PHASE_05_REEL_REGRESSION']
assert not reels['uncaughtExceptions'] and all(a['passed'] for a in reels['assertions'])
assert sum(a['name'].startswith('actual client rotation direction: ') for a in reels['assertions'])==3
...
summary={'passed':True,'tests':counts,'totalTests':sum(counts.values()),'imageCount':len(images),'soundEventCount':len(registry),'suppliedAudio':supplied,'runtimeAssertions':len(results['PHASE_05']['assertions']),'phase01Regression':True,'reelRegression':True,'barBigRuntime':True,'barRegRuntime':True,'buildHashes':results['PHASE_05']['buildHashes']}
