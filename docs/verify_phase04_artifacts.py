...
results={phase:read(root/f'runtime-evidence/{phase}/result.json') for phase in ['PHASE_04','PHASE_04_PHASE01_REGRESSION']}
...
assert len(results['PHASE_04']['assertions']) >= 90
with ZipFile(root/'fabric/build/libs/piri-juggler-fabric-1.0.0.jar') as z:
...
summary={'passed':True,'tests':counts,'totalTests':sum(counts.values()),'imageCount':len(images),'soundEventCount':len(registry),'suppliedAudio':supplied,'runtimeAssertions':len(results['PHASE_04']['assertions']),'phase01Regression':True,'buildHashes':results['PHASE_04']['buildHashes']}
(root/'runtime-evidence/PHASE_04/artifact-verification.json').write_text(json.dumps(summary,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
