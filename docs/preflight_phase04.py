...
patterns={'GRAPE':('GRAPE',)*3,'BELL':('BELL',)*3,'PIERO':('PIERO',)*3,'REPLAY':('REPLAY',)*3,'BIG_ENTRY':('SEVEN',)*3,'REG_ENTRY':('SEVEN','SEVEN','BAR'),'BONUS':('BAR',)*3}
...
for stops in itertools.product(range(21),repeat=3):
    ...
    if not any(cherry):
        ...
        if not any(wins.values()):candidates['MISS'].append(stops);rank['MISS',stops]=5;candidates['BONUS'].append(stops);rank['BONUS',stops]=5
...
minimum={'GRAPE':750,'BELL':50,'PIERO':20,'REPLAY':525,'CHERRY':1510,'MISS':5242,'BONUS':5250,'BIG_ENTRY':10,'REG_ENTRY':10,'PREMIUM_B':1}
...
failures=[]
for role in ['BONUS','CHERRY','PIERO']:
