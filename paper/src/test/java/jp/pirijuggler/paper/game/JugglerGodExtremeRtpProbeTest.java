...
class JugglerGodExtremeRtpProbeTest extends GameFixture {
    @Test void printHundredMillionGameRtpForSelectedSetting(){
        ...
        var result=JugglerGodExtremeSimulator.run(
                weights,setting,100_000_000L,bonus,small,
                new SplittableRandom(0x45585452454d4500L+setting));

        System.out.printf(
                "EXTREME_RTP_100M setting=%d games=%d target=%.3f observed=%.6f delta=%+.6f god=%d godInGod=%d bet=%d payout=%d%n",
                setting,result.leverGames(),target[setting],result.payoutPercent(),
                result.payoutPercent()-target[setting],result.god(),result.godInGod(),
                result.totalBet(),result.totalPayout());
