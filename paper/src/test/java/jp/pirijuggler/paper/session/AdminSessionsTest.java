...
class AdminSessionsTest {
    @Test void validatesOwnerMachineSequenceOpAndExpiryEveryTime() {
        ...
        assertEquals(0, entry.lastSequence()); assertEquals(300100, entry.expiresAt());
        ...
        var next = sessions.open(player, 1, 200); sessions.expire(300200);
    ...
    @Test void newEntryCloseAndRestartInvalidateOldId() {
        ...
        sessions.close(player); assertThrows(DomainException.class, () -> sessions.accept(player, true, fresh.id(), 1, 1, 2));
