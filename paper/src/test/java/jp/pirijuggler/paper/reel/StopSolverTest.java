...
class StopSolverTest {
    ...
    @Test void everyTripletAndExactStrictCountsMatchIndependentEnumeration() throws Exception {
        ...
        var expected=Map.of(DisplayRole.GRAPE,750,DisplayRole.BELL,50,DisplayRole.PIERO,20,DisplayRole.REPLAY,525,DisplayRole.CHERRY,1510,DisplayRole.MISS,5242,DisplayRole.BONUS,5250,DisplayRole.BIG_ENTRY,10,DisplayRole.REG_ENTRY,10,DisplayRole.PREMIUM_B,774);
        ...
        for(var role:DisplayRole.values()){assertEquals(lock.getAsJsonObject("strictCandidateMinimums").get(role.name()).getAsInt(),role.minimum());for(var candidate:CATALOGUE.candidates(role))assertTrue(oracle(candidate.stops()).contains(role));}
    ...
    private static Set<DisplayRole> oracle(StopTriplet s){
        ...
        for(int[] row:rows){String shape=FixedReels.row(Reel.LEFT,s.left(),row[0])+"/"+FixedReels.row(Reel.CENTER,s.center(),row[1])+"/"+FixedReels.row(Reel.RIGHT,s.right(),row[2]);DisplayRole r=switch(shape){case "GRAPE/GRAPE/GRAPE"->DisplayRole.GRAPE;case "BELL/BELL/BELL"->DisplayRole.BELL;case "PIERO/PIERO/PIERO"->DisplayRole.PIERO;case "REPLAY/REPLAY/REPLAY"->DisplayRole.REPLAY;case "SEVEN/SEVEN/SEVEN"->DisplayRole.BIG_ENTRY;case "SEVEN/SEVEN/BAR"->DisplayRole.REG_ENTRY;case "BAR/BAR/BAR"->DisplayRole.BONUS;default->null;};if(r!=null)wins.merge(r,1,Integer::sum);}
        ...
        if(!top&&!middle&&!bottom){if(wins.isEmpty()){result.add(DisplayRole.MISS);result.add(DisplayRole.BONUS);}else if(wins.size()==1&&wins.values().iterator().next()==1)result.add(wins.keySet().iterator().next());}
