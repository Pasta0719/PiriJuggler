...
通常回転方向はindex減少方向（画面上では下向き）。プレミア逆回転の最初の500msはindex増加方向（画面上では上向き）。固定リール配列とTOP/MIDDLE/BOTTOMの定義は変更しない。
...
BAR-BAR-BAR
```
5本の有効ラインのいずれか1本に揃う。BIG/REG共通の当選告知出目であり、ボーナス入賞形ではない。払い出し0枚、Piri Chance点灯、ボーナス権利を保持する。BIG/REGの種類は公開せず、BONUS_STARTも送らない。ハズレゲームでは5本のどの有効ラインにもBAR-BAR-BARを成立させてはいけない。REG入賞形SEVEN-SEVEN-BARは変更しない。
...
BAR_CONFIRMは有効ライン上のBAR-BAR-BARを表す。以下のBONUS候補を除き、すべての候補でBAR_CONFIRMが0本であることを追加条件とする。小役の払い出しやボーナス入賞形とBAR確定目を混在させない。
...
BONUS候補（単独BIG/REG当選ゲーム共通）:
- MISS候補、または次のBAR確定形の和集合
- BAR確定形はBAR_CONFIRMちょうど1本
- GRAPE / BELL / PIERO / REPLAY / BIG / REG が0本
- leftTopCherry=false、leftMiddleCherry=false、leftBottomCherry=false
- 払い出し0枚。BIG/REGで同じ候補集合・tie-breakを使う
- 固定配列の厳密BAR確定形は8件。起動時に8件存在することを検証する
...
BONUS候補内のBAR確定形は成立ラインのrank 0..4を使い、MISS形はrank5を使う。候補追加のための再抽選は行わない。
...
BIG/REG単独:
- 通常ゲームではBIG/REG共通のBONUS候補（MISS形またはBAR-BAR-BAR確定形）を使う
- どちらも払い出し0枚、ゲーム完了でPiri Chance点灯、内部BONUS_PENDING_BIG/REGを保持し公開値はBONUS_PENDING
- 重複役は上記のCHERRY/PIERO候補と2枚/14枚払出しを維持し、BAR揃いへ置換しない
...
上記はサーバー内部とDB保存用のGameStateである。PUBLIC_STATE.gameStateの公開値は第92章の変換規則に従う。
...
LEVER ON:
1. 3リールを逆方向（index増加・画面上向き）12 symbols/secで500ms
2. 500ms経過時に通常方向へ切替
3. 切替瞬間Piri Chance点灯
4. 告知音なし
...
固定リール配列について、`BIG/BONUS`, `CHERRY_BIG/CHERRY`, `PIERO_BIG/PIERO`の3 base候補すべてで、全6停止順・1本目pressedIndex 0..20・2本目pressedIndex 0..20に対して2本目filter後候補が1件以上存在することを起動時self-testとunit testで検証する。1件でも失敗した場合gameplay disabled + SEVERE。
...
Fabricは現在の表示位相から通常回転方向（index減少・画面下向き）に指定stopIndexへアニメーションして停止。距離は `(currentPhase - stopIndex + 21) mod 21` とし、位相をこの距離だけ減らす。
...
`gameState`は公開用状態文字列であり、内部GameStateを直接serializeしない。専用PublicGameStateまたは同等の明示的mappingを使用する。

| 内部GameState | PUBLIC_STATE.gameState |
|---|---|
...
この変換は通信時だけ行い、Paper内部GameState、DB保存値、第18・131章の遷移、BET額は変更しない。
ボーナス入賞ゲームの第3STOPで777または77BARが実際に表示され、種類が視覚的に判明した後は、BIG_READY / BIG_BETTED / BIG_SPINNINGおよびREG_READY / REG_BETTED / REG_SPINNINGを公開してよい。それ以外の公開状態は内部GameState名と同じ文字列を使用する。
BAR-BAR-BARはBIG/REG共通の確定目であり、種類が視覚的に判明した扱いにしない。公開状態はBONUS_PENDING、bonusCountは0、種類やBONUS_STARTを送らない。
Fabricは公開値を表示・入力受付に使い、BIG/REG種類を推測して内部状態を補完しない。第101章の秘匿要件を優先する。
...
PUBLIC_STATE.gameStateも第92章の公開値へ変換し、入賞前の状態名からBIG/REG種類を識別できないようにする。
...
53. ユーザー提供OGGの登録・読み込み・再生経路、音源未配置時の継続、画像assets SHA
